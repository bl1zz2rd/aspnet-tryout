...
{
    public abstract class MultitenantProjTestBase : AbpIntegratedTestBase<MultitenantProjTestModule>
    {
        private DbConnection _hostDb;
        private Dictionary<int, DbConnection> _tenantDbs; //only used for db per tenant architecture

        protected MultitenantProjTestBase()
        {
        ...
        protected void UsingDbContext(Action<MultitenantProjDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<MultitenantProjDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<MultitenantProjDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<MultitenantProjDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<MultitenantProjDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<MultitenantProjDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<MultitenantProjDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<MultitenantProjDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<MultitenantProjDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<MultitenantProjDbContext, Task<T>> func)
        {
            T result;
